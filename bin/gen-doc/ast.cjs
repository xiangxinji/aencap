  ...
  try {
    ...
    for (i in node) {
      ...
      for (c in comments) {
        ...
        const returnTextNode = createTextNode(
          `返回值： ${returns
            .map((i) => {
...
function deepReader(filename, level, parentPath, { parentFileName, node }) {
  ...
  if (stat.isDirectory()) {
    ...
    children.forEach((i) =>
      deepReader(i, level + 1, path.join(parentPath, filename), {
    ...
    if (isParentModules) {
      node.children.push(
        createHeadingNode(level - 1, excludeFileSuffix(filename))
      );
    } else {
      const newNode = createTextNode(
        getCommentCode(path.join(parentPath, filename))
      );
      node.children.push(newNode);
    }
    if (!hasModules) {
      // 核心代码在 index.ts
      const nodes = getExposeFunctions(
