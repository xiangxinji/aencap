 ...
 * range 区域操作
 */
export function useNumberScope(
  min: number,
  max: number,
  { loop = false, onChange = null as Function | null, onInitState = null as Function | null } = {}
): [
...
function computedCurrentIndexLoop(isLoop = false, current: number, min: number, max: number): number {
    if (!isLoop) {
      ...
      return current >= max ? min : current < min ? max : current;
    }
  }
